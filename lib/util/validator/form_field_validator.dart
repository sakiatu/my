...
class MyFormFieldValidator {
  ...
  factory MyFormFieldValidator.fromRules(List<MyFormFieldValidationRule> rules) {
    return MyFormFieldValidator._([...rules]); // Create a copy to ensure immutability
  }
  ...
  MyFormFieldValidator minLength(int minLength, {String? message}) {
    return MyFormFieldValidator._([...rules, MinLengthRule(minLength, message: message)]);
  }

  /// Adds a [MaxLengthRule] to the validator chain.
  MyFormFieldValidator maxLength(int maxLength, {String? message}) {
    return MyFormFieldValidator._([...rules, MaxLengthRule(maxLength, message: message)]);
  }
  ...
  MyFormFieldValidator numberRange({required num min, required num max, String? message}) {
    return MyFormFieldValidator._([...rules, NumberRangeRule(min: min, max: max, message: message)]);
  }

  /// Adds a [RegexRule] to the validator chain.
  MyFormFieldValidator matchesRegex(RegExp regex, {required String message}) {
    return MyFormFieldValidator._([...rules, RegexRule(regex, message: message)]);
  }

  /// Adds a [PasswordComplexityRule] to the validator chain.
  MyFormFieldValidator passwordComplexity({String? message}) {
    return MyFormFieldValidator._([...rules, PasswordComplexityRule(message: message)]);
  }
  ...
  MyFormFieldValidator confirmsPassword(ValueGetter<String?> passwordValueGetter, {String? message}) {
    return MyFormFieldValidator._(
        [...rules, ConfirmPasswordRule(passwordValueGetter: passwordValueGetter, message: message)]);
  }

  /// Adds a custom [MyFormFieldValidationRule] to the validator chain.
  MyFormFieldValidator add(MyFormFieldValidationRule rule) => MyFormFieldValidator._([...rules, rule]);
  ...
  FormFieldValidator<String> getValidator({BuildContext? context}) => (String? value) {
